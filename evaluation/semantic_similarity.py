...
def evaluate_dataset():
    ...
    with open("cohere_evaluation/webdev_fewshot_results.json", "r") as file:
        summaries = json.load(file)

    results = []
    for entry in summaries:
        reference = entry.get("reference_summary", "")
        generated = entry["cohere_summary"]  # Can switch to other models

        scores = evaluate_semantic_similarity(reference, generated)
        entry.update(scores)
        results.append(entry)

    with open("cohere_evaluation/webdev1_fewshot_results.json", "w") as file:
