...
dataset = []
with open("data/processed/dataset_webdev.jsonl", "r") as file:
    for line in file:
...
for item in dataset[:5]:  # Testing on 5 samples
    ...
    for prompt_name, prompt_func in prompt_functions.items():
        ...
        print(f"\nGenerated {prompt_name} Prompt:\n", prompt_text)  # Debugging output
        ...
        print(f"\nGenerated Summary for {prompt_name} Prompt:\n", summary["cohere_summary"])  # Debugging output

        results.append(summary)

# Save results
#change .json line to output summaries of different model
with open("results/cohere/webdev_prompting_few_shot.json", "w") as outfile:
